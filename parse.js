...
function process(node, parent, input, globalPostcssSass) {
    ...
    switch (node.type) {
        ...
        case 'value': {
            ...
            let spaceNode;
            node.content.forEach(contentNode => {
                if (contentNode.type === 'important') {
                    parent.raws.important = (spaceNode ? spaceNode.content : '') + contentNode.content;
                    parent.important = true;
                    spaceNode = null;
                    return;
                }
                if (spaceNode) {
                    valueNodes.push(spaceNode);
                    spaceNode = null;
                }
                if (contentNode.type === 'space') {
                    spaceNode = contentNode;
                } else {
                    valueNodes.push(contentNode);
                }
            });
        ...
        case 'multilineComment': {
            const comment = Object.assign(postcss.comment(), {
                raws: {
                    before: globalPostcssSass.before,
                    inline: node.type === 'singlelineComment'
                }
            });
            if (/^\s*$/.test(node.content)) {
                comment.text       = '';
                comment.raws.left  = node.content;
                comment.raws.right = '';
            } else {
                let match = node.content.match(/^(\s*)([^]*[^\s])(\s*)$/);
                let fixed = match[2].replace(/(\*\/|\/\*)/g, '*//*');
                comment.text       = fixed;
                comment.raws.left  = match[1];
                comment.raws.right = match[3];
                comment.raws.text  = match[2];
            }
...
module.exports = function sassParse(source, opts) {
